    ...
    def walk(element, path):
        ...
        for (k, v) in sorted(element.items(), key = lambda x : x[0]):
            if EVEN_VAL in v and 'children' in v: 
                return_str = return_str + walk(v['children'], get_path(path, k))
            if MINUS_VAL in v and PLUS_VAL in v:
                return_str = return_str + 'Property \'{}\' was updated. From {} to {}'.format(get_path(path, k), encode_value(v, MINUS_VAL), encode_value(v, PLUS_VAL)) + '\n'
            else:
                if MINUS_VAL in v:
                    return_str = return_str + 'Property \'{}\' was removed'.format(get_path(path, k)) + '\n'
                if PLUS_VAL in v:
                    return_str = return_str + 'Property \'{}\' was added with value: {}'.format(get_path(path, k), encode_value(v, PLUS_VAL)) + '\n'
        return return_str              
